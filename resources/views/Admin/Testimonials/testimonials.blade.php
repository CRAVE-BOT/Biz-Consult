...
@section('content')
    ...
    <a href="{{route('admin-testimonials-create')}}" class="btn btn-primary w-100 d-flex align-items-center justify-content-center">
        {{__('keywords.Add new testimonials')}}
    </a>
    <br> <br>
          ...
          @foreach($testimonials as $row)
            <tr>
                ...
                <td class="text-nowrap">
                    <a href="{{route('admin-testimonials-show',$row)}}" class="btn btn-sm btn-primary me-1">{{__('keywords.Show')}}</a>
                    <a href="{{route('admin-testimonials-edit',$row)}}" class="btn btn-sm btn-warning me-1">{{__('keywords.update')}}</a>
                    <form action="{{route('admin-testimonials-destroy',$row)}}" method="POST" style="display:inline-block;">
                        @csrf
                        @method('DELETE')
